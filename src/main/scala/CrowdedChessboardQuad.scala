...
object CrowdedChessboardQuad extends App {
  ...
  for(i <- 0 until n-1; j <- 0 until n) {
    ...
    for((f, c) <- moviments; if f < n && c >= 0 && c < n) e.addAMOQuad(List(taulerCavalls(i)(j), taulerCavalls(f)(c)))
  }
  ...
  for(i <- 0 until n; j <- 0 until n) e.addAMOQuad(List(taulerReines(i)(j), taulerTorres(i)(j), taulerAlfils(i)(j), taulerCavalls(i)(j)))
