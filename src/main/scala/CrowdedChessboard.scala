...
object CrowdedChessboard extends App {
  ...
  def clausulesAlfils(tauler:Array[Array[Int]], opcio:Int) = {
    ...
    /* if(opcio == 1) {
      e.addEOQuad(List(tauler(0)(0), tauler(n - 1)(n - 1)))
      e.addEOQuad(List(tauler(n - 1)(0), tauler(0)(n - 1)))
    }
    else {
      e.addEOLog(List(tauler(0)(0), tauler(n - 1)(n - 1)))
      e.addEOLog(List(tauler(n - 1)(0), tauler(0)(n - 1)))
    }

    for(i <- 1 until n-1; j <- 1 until n-1) {
      e.addClause(-tauler(i)(j) :: List())
    }


    for (i <- 1 until n-1) {
      if(opcio == 1) { e.addEOQuad(List(tauler(0)(i), tauler(i)(0))); e.addEOQuad(List(tauler(n-1)(i), tauler(i)(n-1)));  e.addEOQuad(List(tauler(0)(i), tauler(n-1-i)(n-1))); e.addEOQuad(List(tauler(i)(0), tauler(n-1)(n-1-i)));}
      else { e.addEOLog(List(tauler(0)(i), tauler(i)(0))); e.addEOQuad(List(tauler(n-1)(i), tauler(i)(n-1))); e.addEOLog(List(tauler(0)(i), tauler(n-1-i)(n-1))); e.addEOLog(List(tauler(i)(0), tauler(n-1)(n-1-i))); }
    } */
